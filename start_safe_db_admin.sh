...
echo -e "${BLUE}==========================================${NC}"
echo -e "${BLUE}   未决定AI平台 - 安全数据库管理工具   ${NC}"
echo -e "${BLUE}==========================================${NC}"
...
( tail -f logs/safe_db_admin.log & ) 2>/dev/null
LOG_PID=$!
...
# 设置关键环境变量，避免加载视频处理库
export IMAGEIO_NO_IMPORT=1
export OPENCV_DISABLE_GLOBAL_RESOURCES=1

# 启动安全数据库管理工具
echo -e "${GREEN}启动安全数据库管理工具...${NC}"
python3 safe_db_admin.py
